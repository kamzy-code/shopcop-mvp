...
import { LuShieldCheck, LuStore } from 'react-icons/lu';
...
export default function AdminOnboardingPage() {
  ...
  // Pre-fill if partially saved before
  useEffect(() => {
    if (!profile) return;
    reset({
  ...
  const onSubmit = async (data: AdminProfileFormData) => {
    ...
    try {
      ...
      // MutationErrorAlert handles display
    }
  };
      ...
      <Box flex={1} overflow="auto">
        <Center py={{ base: 6, md: 10 }} px={{ base: 4, sm: 6 }}>
          <Stack w="full" maxW="640px" gap={0}>
            <FormCard
              icon={<LuShieldCheck size={20} color="var(--chakra-colors-primary-600)" />}
              title="Set Up Your Admin Profile"
              description="Complete your profile before accessing the admin panel. First name, last name, and phone number are required."
            >
